# Egroo Product Showcase

This page is a visual tour of the current Egroo experience. It highlights the chat surface, built-in AI agent workflows, multimodal prompts, session management, and real-time voice calling.

## At A Glance

| Area | What it shows |
|---|---|
| Agent mentions | Calling an agent directly from the conversation composer |
| Agent replies | A generated response rendered inline in the timeline |
| Typing state | Live feedback while an agent is generating a reply |
| Multimodal prompts | Image attachments flowing into an agent conversation |
| Agent management | Session visibility and built-in tool configuration |
| Voice calls | Real-time calling directly inside group chat |
...
These screens show the shape of the product in practice:

- Real-time chat stays central.
- AI agents remain part of the conversation, not a detached sidebar tool.
- Multimodal prompts fit naturally into the message flow.
- Session visibility gives users control over agent memory.
- Voice communication is available from the same collaboration surface.

For the project overview, architecture diagrams, and setup instructions, go back to the [README](https://github.com/jihadkhawaja/Egroo/blob/main/README.md).